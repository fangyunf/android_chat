...
public class IMUtil {
    public static void loginIM(Activity context, String account, String token) {
        ...
        LoginInfo loginInfo = LoginInfo.LoginInfoBuilder.loginInfoDefault(account, token).withAppKey(DataUtils.readAppKey(context)).build();
...
//        bean.token = token;
        HttpUtil.apiW().home_getUserByToken(bean).enqueue(new CommonCallback<NetData>() {
            @Override
...
//        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
    ...
    public static void loginOut(Activity activity) {
        IMKitClient.logoutIM(new com.netease.yunxin.kit.corekit.im.login.LoginCallback<Void>() {
            @Override
            public void onError(int errorCode, @NonNull String errorMsg) {
                Toast.makeText(activity, "error code is " + errorCode + ", message is " + errorMsg, Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onSuccess(@Nullable Void data) {
                if (activity.getApplicationContext() instanceof IMApplication) {
                    ((IMApplication) activity.getApplicationContext()).clearActivity(activity);
                }
