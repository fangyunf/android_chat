...
//                colorSpan, segment.start, segment.end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    /**
     * 同一会话在短时间内的防重：避免 onFailed 与 msgStatusObserver 重复插入 tip
     */
    private static final java.util.Map<String, Long> notFriendTipInsertTimeBySession = new java.util.concurrent.ConcurrentHashMap<>();
    private static final long NOT_FRIEND_TIP_DEBOUNCE_MS = 3000;

    /**
     * 非好友发送失败时插入本地 tip：提示先添加好友，点击可跳转添加好友页。
     * 仅用于 P2P 单聊。
     *
     * @return 已保存的 tip 消息，用于 UI 展示；防重跳过时返回 null
     */
    public static IMMessage saveLocalNotFriendTipMessageAndNotify(IMMessage message) {
        String sessionId = message.getSessionId();
        long now = SystemClock.elapsedRealtime();
        Long last = notFriendTipInsertTimeBySession.get(sessionId);
        if (last != null && (now - last) < NOT_FRIEND_TIP_DEBOUNCE_MS) {
            ALog.d(LIB_TAG, TAG, "saveLocalNotFriendTipMessage: skip duplicate for session " + sessionId);
            return null;
        }
        notFriendTipInsertTimeBySession.put(sessionId, now);
        IMMessage tipMsg =
                MessageBuilder.createTipMessage(message.getSessionId(), message.getSessionType());
        tipMsg.setStatus(MsgStatusEnum.success);
        tipMsg.setDirect(message.getDirect());
