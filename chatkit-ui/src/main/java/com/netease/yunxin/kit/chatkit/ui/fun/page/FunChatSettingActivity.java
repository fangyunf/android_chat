...
public class FunChatSettingActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected final EventNotify<CloseChatPageEvent> closeEventNotify = new EventNotify<CloseChatPageEvent>() {
        @Override
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding.tvCopy.setOnClickListener(view -> {
            if (userBean != null) {
                ClipboardManager clipboard = (ClipboardManager) FunChatSettingActivity.this.getSystemService(Context.CLIPBOARD_SERVICE);
                // 创建一个ClipData对象，包含要复制的文本
                ClipData clip = ClipData.newPlainText("label", userBean.memberCode);
                // 将ClipData对象放入剪切板
    ...
    private void initRequest() {
        ...
        viewModel1.getFriendFetchResult().observe(this, mapFetchResult -> {
    ...
    private void initView() {
        ...
        binding.funChatSettingActivityMemo.funTitleTfArrowViewLl.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ...
                intent.putExtra(FunCommentActivity.REQUEST_COMMENT_NAME_KEY, userInfoData.friendInfo.getAlias().isEmpty() ? userInfoData.friendInfo.getName() : userInfoData.friendInfo.getAlias());
...
//        binding.funChatSettingActivityRecommand.titTv.setText("推荐好友");
        ...
        binding.funChatSettingActivityToTop.funTitleTfArrowViewSwitch.setOnClickListener(v -> {
                    if (!binding.funChatSettingActivityToTop.funTitleTfArrowViewSwitch.isSelected()) {
                        ConversationRepo.addStickTop(accId, SessionTypeEnum.P2P, "", new ChatCallback<StickTopSessionInfo>() {
                            @Override
                        ...
                        ConversationRepo.removeStickTop(accId, SessionTypeEnum.P2P, "", new ChatCallback<Void>() {
                            @Override
        ...
        binding.funChatSettingActivityClearHistory.funTitleTfArrowViewLl.setOnClickListener(v -> {
//                ALog.d("123");
            DialogAlertUtil.showAlert("确认删除聊天记录吗？", type -> {
                if (type == 1) {
        ...
        binding.funChatSettingActivityClearDontNoticeMsg.funTitleTfArrowViewSwitch.setOnClickListener(v -> ConversationRepo.setNotify(accId, SessionTypeEnum.P2P, binding.funChatSettingActivityClearDontNoticeMsg.funTitleTfArrowViewSwitch.isSelected(), new ChatCallback<Void>() {
            @Override
            public void onSuccess(@Nullable Void param) {
        ...
        binding.funChatSettingActivityClearAddBlackList.funTitleTfArrowViewSwitch.setOnClickListener((View v) -> {
...
//                        viewModel1.addBlack(accId);
            ...
            HttpUtil.apiW().friends_changeBlackState(registerBean).enqueue(new CommonCallback<NetData>() {
                @Override
                public void Successful(Call<NetData> call, Response<NetData> response, NetData body) {
                ...
                DialogAlertUtil.showAlert("确定删除好友吗？", new DialogAlertUtil.DialogAlertUtilCallBack() {
                    ...
                    public void clickType(int type) {
                        if (type == 1) {
                            ...
                            HttpUtil.apiW().friends_delFriend(bean).enqueue(new CommonCallback<NetData>() {
                                @Override
                                public void Successful(Call<NetData> call, Response<NetData> response, NetData body) {
...
//                viewModel1.deleteFriend(userInfoData.data.getAccount());
        ...
        binding.funChatSettingActivitySendMsgRl.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (type == 1) {
                    ...
                    XKitRouter.withKey(RouterConstant.PATH_FUN_CHAT_P2P_PAGE).withParam(RouterConstant.CHAT_ID_KRY, userInfoData.data.getAccount()).withContext(FunChatSettingActivity.this).navigate();
                    finish();
                } else {
                    finish();
                }
    ...
    private void registerResult() {
        commentLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
            if (result.getResultCode() == BaseCommentActivity.RESULT_OK && result.getData() != null) {
                String comment = result.getData().getStringExtra(BaseCommentActivity.REQUEST_COMMENT_NAME_KEY);
    ...
    private void refreshView() {
        if (friendInfo != null) {
            binding.funChatSettingActivityAvatarView.setData(friendInfo.getAvatar(), friendInfo.getAvatarName(), AvatarColor.avatarColor(friendInfo.getAccount()));
...
//      binding.noTeamNameTv.setText(accId);
            ...
            binding.funChatSettingActivityAvatarView.setData(userInfo.getAvatar(), name, AvatarColor.avatarColor(userInfo.getAccount()));
...
//
    ...
    private void selectUsersCreateGroup() {
        ...
        XKitRouter.withKey(RouterConstant.PATH_FUN_SELECT_CREATE_TEAM_PAGE).withParam(RouterConstant.KEY_CONTACT_SELECTOR_MAX_COUNT, CHAT_P2P_INVITER_USER_LIMIT - 1).withParam(RouterConstant.KEY_REQUEST_SELECTOR_NAME_ENABLE, true).withContext(this).withParam(RouterConstant.SELECTOR_CONTACT_FILTER_KEY, filterList).withParam(RouterConstant.REQUEST_CONTACT_SELECTOR_KEY, filterList).navigate();
    }
