            ...
            if (!messageInfo.getMessage().getAttachStr().isEmpty()) {
                ...
                HttpUtil.apiW().red_checkRedpacet(bean).enqueue(new CommonCallback<NetData>() {
                    ...
                    public void Successful(Call<NetData> call, Response<NetData> response, NetData body) {
                        ...
                        // 处理红包
                        if (redBean.type == 1 || redBean.type == 3) {
                            //可领取
                            if (redBean.type == 1) {
                        ...
                        if (redBean.type == 2 || redBean.type == 4 || redBean.type == 5) {
                            ...
                            if (context != null) {
                                ...
                                // 延迟更新消息，避免闪烁（在下一个UI循环中执行）
                                new Handler().postDelayed(() -> {
                                    HashMap<String, Object> localExtensionMap = new HashMap<>();
                                    localExtensionMap.put("userId", DataUtil.getUserid());
                                    localExtensionMap.put("hasDragDown", 1);
                                    IMMessage message = messageInfo.getMessage();
                                    message.setLocalExtension(localExtensionMap);
