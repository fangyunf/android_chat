...
public class FunChatP2PFragment extends FunChatFragment {
    ...
    protected void initView() {
        super.initView();
        if (DataUtil.getKeFuId().equals(sessionID)) {
            ...
            chatView.getTitleBar().setOnBackIconClickListener(v -> requireActivity().onBackPressed()).setActionImg(R.drawable.ic_more_point).setActionListener(v -> {
                ...
                XKitRouter.withKey(RouterConstant.PATH_FUN_CHAT_SETTING_PAGE).withParam(RouterConstant.CHAT_ID_KRY, sessionID).withContext(requireActivity()).navigate();
            });
            ...
            layoutParams.width = SizeUtils.dp2px(30);
    ...
    protected void initDataObserver() {
        ...
        p2pReceiptObserver = imMessageReceiptInfo -> chatView.getMessageListView().setP2PReceipt(imMessageReceiptInfo.getMessageReceipt().getTime());
        ...
        ((ChatP2PViewModel) viewModel).getFriendInfoLiveData().observe(getViewLifecycleOwner(), result -> {
    ...
    public void onNewIntent(Intent intent) {
        ...
        if (anchorMessage != null) {
            ...
            if (position >= 0) {
                chatView.getMessageListView().getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        chatView.getMessageListView().getViewTreeObserver().removeOnGlobalLayoutListener(this);
