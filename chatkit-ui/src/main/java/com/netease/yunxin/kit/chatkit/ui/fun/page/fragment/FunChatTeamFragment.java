...
public class FunChatTeamFragment extends FunChatFragment {
    ...
    protected void initData(Bundle bundle) {
        ...
        chatView.getTitleBar().setOnBackIconClickListener(v -> requireActivity().onBackPressed()).setActionImg(R.drawable.ic_more_point).setActionListener(v -> {
            // go to team setting
            chatView.hideCurrentInput();
            XKitRouter.withKey(PATH_FUN_TEAM_SETTING_PAGE).withContext(requireContext()).withParam(KEY_TEAM_ID, sessionID).navigate();
        });
...
//    startTimer();
    ...
    void _requestCaiData() {
        ...
        HttpUtil.apiW().caidan_groupCaidan(registerBean).enqueue(new CommonCallback<NetData>() {
            @Override
            public void Successful(Call<NetData> call, Response<NetData> response, NetData body) {
    ...
    private void refreshView() {
        ...
        if (currentMember != null && teamInfo != null) {
            if (currentMember.getType() != TeamMemberType.Owner && currentMember.getType() != TeamMemberType.Manager) {
    ...
    public void onMessageEvent(BaseEvent event) {
        if (event.getTag().equals("clearTeamMessageList")) {
            ...
            if (customMsgBean.groupId.equals(sessionID)) {
                ...
                Glide.with(getContext()).asGif().load(R.drawable.egg_open_success) // 替换为你的本地GIF文件名
    ...
    public void onDestroyView() {
        super.onDestroyView();
        ((ChatTeamViewModel) viewModel).getTeamMessageReceiptLiveData().removeObserver(teamReceiptObserver);
    ...
    protected void initDataObserver() {
        ...
        teamReceiptObserver = listFetchResult -> {
            ...
            for (IMTeamMessageReceiptInfo receiptInfo : listFetchResult.getData()) {
                ...
                for (ChatMessageBean messageBean : messageList) {
                    if (messageBean != null && messageBean.getMessageData() != null && TextUtils.equals(messageBean.getMessageData().getMessage().getUuid(), msgId)) {
                        msg = messageBean;
                        break;
                    }
                }
        ...
        ((ChatTeamViewModel) viewModel).getTeamMessageReceiptLiveData().observeForever(teamReceiptObserver);
        ...
        ((ChatTeamViewModel) viewModel).getTeamMemberData().observe(getViewLifecycleOwner(), teamMembers -> {
        ...
        ((ChatTeamViewModel) viewModel).getTeamMemberChangeData().observe(getViewLifecycleOwner(), result -> {
    ...
    private void startKickDialog() {
        ...
        positiveBut.setOnClickListener(v -> {
            if (alertDialog != null) {
                alertDialog.dismiss();
            }
            requireActivity().finish();
    ...
    public void onNewIntent(Intent intent) {
        ...
        ChatMessageBean anchorMessageBean = (ChatMessageBean) intent.getSerializableExtra(RouterConstant.KEY_MESSAGE_BEAN);
        ...
        if (anchorMessage != null) {
            ...
            if (position >= 0) {
                chatView.getRootView().getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        chatView.getRootView().getViewTreeObserver().removeOnGlobalLayoutListener(this);
                        chatView.getRootView().post(() -> chatView.getMessageListView().scrollToPosition(position));
                    }
                });
