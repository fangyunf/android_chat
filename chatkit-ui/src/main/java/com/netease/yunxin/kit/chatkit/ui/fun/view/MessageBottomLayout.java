...
public class MessageBottomLayout extends FrameLayout
    ...
    private boolean mMuteFullTeam = true;
...
//    mBinding.chatMsgInputSwitchLayout.setVisibility(show ? GONE : VISIBLE);
    ...
    public void setMute(boolean mute, boolean fullTeamMute) {
        boolean fullTeamChanged = mute && fullTeamMute != mMuteFullTeam;
        if (mute == mMute && !fullTeamChanged) {
            return;
        }
        mMute = mute;
        mMuteFullTeam = fullTeamMute;
        ...
        if (mute) {
            mBinding.inputMuteTv.setText(
                    getContext()
                            .getString(
                                    mMuteFullTeam
                                            ? R.string.chat_team_mute_all_banner
                                            : R.string.chat_team_mute_member_banner));
            ...
            mBinding.chatInputEditorContainer.setVisibility(GONE);
            mBinding.inputAudioTv.setVisibility(GONE);
            mBinding.inputLayout.setBackgroundColor(Color.TRANSPARENT);
            mBinding.inputBarLayout.setBackgroundColor(Color.TRANSPARENT);
            clearReplyMsg();
        } else {
            mMuteFullTeam = true;
            ...
            mBinding.chatInputEditorContainer.setVisibility(VISIBLE);
            mBinding.inputLayout.setBackgroundResource(R.color.color_white);
            mBinding.inputBarLayout.setBackgroundResource(
                    com.yaoxin.appbase.R.drawable.bg_f4f4f4_rounded_22);
            recordShow(false, 0);
            updateState(InputState.input);
        }
        mBinding.inputEt.setText("");
