...
public class MessageBottomLayout extends FrameLayout implements IAudioRecordCallback, AitTextChangeListener, IItemActionListener {
    ...
    public void init(IMessageProxy proxy) {
        ...
        mBinding.inputAudioTv.setOnTouchListener((v, event) -> {
...
//        });
    ...
    private void initView() {
        mBinding = FunChatMessageBottomViewBinding.inflate(LayoutInflater.from(getContext()), this, true);
        ...
        mBinding.inputEt.setOnTouchListener((v, event) -> {
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                switchInput();
            }
            return false;
        ...
        mBinding.btnSend.setVisibility(TextUtils.isEmpty(mBinding.inputEt.getText()) ? View.GONE : View.VISIBLE);

        mBinding.emojiPickerView.setWithSticker(true);
        // 多行消息设置点击事件
        mBinding.chatRichEt.addTextChangedListener(new TextWatcher() {
            @Override
    ...
    private TextWatcher msgInputTextWatcher = new TextWatcher() {
        private int start;
        private int count;
        ...
        public void afterTextChanged(Editable s) {
            ...
            mBinding.btnSend.setVisibility(TextUtils.isEmpty(s) ? View.GONE : View.VISIBLE);
        }
    };
...
//    mBinding.chatMsgInputSwitchLayout.setOnClickListener(listener);
    ...
    public void switchRichInput(boolean titleForces, String title, String content) {
        ...
        if (!TextUtils.isEmpty(title)) {
            ...
            MessageHelper.identifyFaceExpression(getContext(), mBinding.chatRichEt, title, ImageSpan.ALIGN_BOTTOM);
        } else {
            mBinding.chatRichEt.setText("");
            mBinding.chatRichEt.setVisibility(GONE);
        }
        MessageHelper.identifyExpressionForEditMsg(getContext(), mBinding.inputEt, content, aitTextWatcher != null ? aitTextWatcher.getAitContactsModel() : null);
...
//    mBinding.chatMsgInputSwitchLayout.setVisibility(show ? GONE : VISIBLE);
    ...
    public void morePanelShow(boolean show, long delay) {
        ...
        if (!mActionsPanel.hasInit() && show) {
            mActionsPanel.init(mBinding.actionsPanelVp, FunBottomActionFactory.assembleInputMoreActions(mProxy.getSessionType()), this);
        }
    ...
    public void onCameraClick() {
        BottomChoiceDialog dialog = new BottomChoiceDialog(this.getContext(), FunBottomActionFactory.assembleTakeShootActions());
        dialog.setOnChoiceListener(new BottomChoiceDialog.OnChoiceListener() {
            @Override
            public void onChoice(@NonNull String type) {
                if (!XKitUtils.getApplicationContext().getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_ANY)) {
    ...
    public void onCallClick() {
        BottomChoiceDialog dialog = new BottomChoiceDialog(this.getContext(), FunBottomActionFactory.assembleVideoCallActions());
        dialog.setOnChoiceListener(new BottomChoiceDialog.OnChoiceListener() {
            @Override
            public void onChoice(@NonNull String type) {
    ...
    public void onLocationClick() {
        ...
        if (PermissionUtils.hasPermissions(IMKitClient.getApplicationContext(), permissions)) {
            ...
            Permission.requirePermissions(IMKitClient.getApplicationContext(), permissions).request(new Permission.PermissionCallback() {
                @Override
                ...
                public void onDenial(List<String> permissionsDenial, List<String> permissionDenialForever) {
                    Toast.makeText(getContext(), R.string.permission_default, Toast.LENGTH_SHORT).show();
                }
...
//      mBinding.inputMoreRb.setAlpha(mute ? 0.5f : 1f);
    ...
    public void setReplyMessage(ChatMessageBean messageBean) {
        ...
        MessageHelper.identifyFaceExpression(getContext(), mBinding.replyContentTv, tips, ImageSpan.ALIGN_CENTER, MessageHelper.SMALL_SCALE);
    ...
    public void onRecordSuccess(File audioFile, long audioLength, RecordType recordType) {
        ALog.d(LIB_TAG, TAG, "onRecordSuccess -->> file:" + audioFile.getName() + " length:" + audioLength);
        endRecord();
