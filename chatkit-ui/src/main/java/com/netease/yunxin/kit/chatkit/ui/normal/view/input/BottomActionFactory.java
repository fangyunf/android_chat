...
public class BottomActionFactory {
    ...
    public static List<ActionItem> assembleInputMoreActions(SessionTypeEnum sessionType) {
        ArrayList<ActionItem> actions = new ArrayList<>();
        actions.add(new ActionItem(ActionConstants.ACTION_TYPE_CAMERA, R.drawable.ic_shoot, R.string.chat_message_more_shoot));
        actions.add(new ActionItem(ActionConstants.ACTION_TYPE_LOCATION, R.drawable.ic_location, R.string.chat_message_location));
        actions.add(new ActionItem(ActionConstants.ACTION_TYPE_FILE, R.drawable.ic_send_file, R.string.chat_message_file));
        if (sessionType == SessionTypeEnum.P2P) {
            actions.add(new ActionItem(ActionConstants.ACTION_TYPE_VIDEO_CALL, R.drawable.ic_video_call, R.string.chat_message_video_call));
        }
        if (ChatKitClient.getChatUIConfig() != null && ChatKitClient.getChatUIConfig().chatInputMenu != null) {
