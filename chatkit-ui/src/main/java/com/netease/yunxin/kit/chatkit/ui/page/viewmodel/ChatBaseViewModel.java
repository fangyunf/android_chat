...
public abstract class ChatBaseViewModel extends BaseViewModel {
    ...
    private final FetchResult<List<ChatMessageBean>> messageRecFetchResult = new FetchResult<>(LoadStatus.Finish);
    private final MutableLiveData<FetchResult<List<String>>> userInfoLiveData = new MutableLiveData<>();
    private final FetchResult<List<String>> userInfoFetchResult = new FetchResult<>(LoadStatus.Finish);
    private final MutableLiveData<FetchResult<Map<String, MsgPinOption>>> msgPinLiveData = new MutableLiveData<>();
    private final FetchResult<Map<String, MsgPinOption>> msgPinFetchResult = new FetchResult<>(LoadStatus.Finish);
    ...
    private final MutableLiveData<FetchResult<List<ChatMessageBean>>> deleteMessageLiveData = new MutableLiveData<>();
    ...
    private final EventObserver<IMMessageInfo> msgStatusObserver = new EventObserver<IMMessageInfo>() {
        ...
        public void onEvent(@Nullable IMMessageInfo event) {
            ALog.d(LIB_TAG, TAG, "msg status change -->> " + (event == null ? "null" : event.getMessage().getStatus()));
    ...
    private final EventObserver<List<IMMessageInfo>> msgSendingObserver = new EventObserver<List<IMMessageInfo>>() {
        @Override
        public void onEvent(@Nullable List<IMMessageInfo> event) {
            ALog.d(LIB_TAG, TAG, "msg sending change -->> " + (event == null ? "null" : event.size()));
    ...
    private final Observer<AttachmentProgress> attachmentProgressObserver = attachmentProgress -> {
        ALog.d(LIB_TAG, TAG, "attachment progress update -->> " + attachmentProgress.getTransferred() + "/" + attachmentProgress.getTotal());
    ...
    private final EventObserver<IMMessageInfo> deleteMsgObserver = new EventObserver<IMMessageInfo>() {
        @Override
        public void onEvent(@Nullable IMMessageInfo event) {
            ALog.d(LIB_TAG, TAG, "msg delete -->> " + (event == null ? "null" : event.getMessage().getUuid()));
            if (event != null && TextUtils.equals(event.getMessage().getSessionId(), mSessionId)) {
                ChatMessageBean messageBean = new ChatMessageBean(new IMMessageInfo(event.getMessage()));
    ...
    private final EventObserver<List<IMMessageInfo>> deleteMsgBatchObserver = new EventObserver<List<IMMessageInfo>>() {
        @Override
        public void onEvent(@Nullable List<IMMessageInfo> event) {
            ...
            if (event != null) {
                ...
                for (IMMessageInfo messageInfo : event) {
                    ...
                    if (TextUtils.equals(messageInfo.getMessage().getSessionId(), mSessionId)) {
                        ChatMessageBean messageBean = new ChatMessageBean(new IMMessageInfo(messageInfo.getMessage()));
    ...
    private final Observer<RevokeMsgNotification> revokeMsgObserver = revokeMsgNotification -> {
        ALog.d(LIB_TAG, TAG, "revokeMsgObserver");
        ChatMessageBean messageBean = new ChatMessageBean(new IMMessageInfo(revokeMsgNotification.getMessage()));
    ...
    private final Observer<FriendChangedNotify> friendChangedObserver = friendChangedNotify -> {
        if (friendChangedNotify != null) {
            ...
            for (Friend friend : friendChangedNotify.getAddedOrUpdatedFriends()) {
                ...
                ALog.d(LIB_TAG, TAG, "friendChangedObserver,AddedOrUpdatedFriends:" + friend.getAccount());
            }
    ...
    private final EventNotify<PinEvent> localPin = new EventNotify<PinEvent>() {
        @Override
    ...
    public void deleteMessage(List<ChatMessageBean> messageList) {
        ...
        for (ChatMessageBean messageBean : messageList) {
            if (messageBean.getMessageData().getMessage().getServerId() == 0L || messageBean.getMessageData().getMessage().getStatus() == MsgStatusEnum.fail) {
        ...
        ChatRepo.deleteMessage(deleteList, null, new FetchCallbackImpl<Long>() {
            @Override
            public void onSuccess(@Nullable Long param) {
                ALog.d(LIB_TAG, TAG, "deleteMessage,onSuccess:" + String.valueOf(param != null ? param : 0));
                doActionAfterDelete(messageList);
            }
    ...
    public void revokeMessage(ChatMessageBean messageBean) {
        if (messageBean != null && messageBean.getMessageData() != null) {
            ...
            ChatRepo.revokeMessage(messageBean.getMessageData(), new FetchCallback<Void>() {
                @Override
                public void onSuccess(@Nullable Void param) {
                    ...
                    if (ChatConfigManager.enableInsertLocalMsgWhenRevoke) {
                        ...
                        fetchResult.setData(messageBean);
                        // 他人撤回消息底层会收到通知进行处理，并保存到本地。当前账号的撤回需要自行处理
                        MessageHelper.saveLocalRevokeMessage(messageBean.getMessageData().getMessage(), true);
                        revokeMessageLiveData.setValue(fetchResult);
                    } else {
                        FetchResult<List<ChatMessageBean>> result = new FetchResult<>(LoadStatus.Success);
                        result.setData(Collections.singletonList(messageBean));
                ...
                public void onFailed(int code) {
                    ...
                    fetchResult.setError(code, code == ResponseCode.RES_OVERDUE ? R.string.chat_message_revoke_over_time : R.string.chat_message_revoke_error);
                    revokeMessageLiveData.setValue(fetchResult);
    ...
    public void init(String sessionId, SessionTypeEnum sessionType) {
        ...
        SettingRepo.getShowReadStatus(new FetchCallbackImpl<Boolean>() {
            @Override
    ...
    public void sendRichTextMessage(String title, String content, List<String> pushList, Map<String, Object> remoteExtension) {
        ALog.d(LIB_TAG, TAG, "sendTextMessage:" + (content != null ? content.length() : "null"));
    ...
    public void sendTextMessage(String content, List<String> pushList, Map<String, Object> remoteExtension) {
        ALog.d(LIB_TAG, TAG, "sendTextMessage:" + (content != null ? content.length() : "null"));
    ...
    public void addMsgCollection(IMMessageInfo messageInfo) {
        ...
        ChatRepo.collectMessage(messageInfo.getMessage(), new ChatCallback<CollectInfo>().setShowSuccess(true));
    }
    ...
    public void sendForwardMessage(ChatMessageBean message, String inputMsg, String sessionId, SessionTypeEnum sessionType) {
        ALog.d(LIB_TAG, TAG, "sendForwardMessage:" + sessionId);
        if (!message.isRevoked()) {
            IMMessage forwardMessage = MessageBuilder.createForwardMessage(message.getMessageData().getMessage(), sessionId, sessionType);
        ...
        if (!TextUtils.isEmpty(inputMsg) && TextUtils.getTrimmedLength(inputMsg) > 0) {
            new Handler(Looper.getMainLooper()).postDelayed(() -> sendTextMessage(inputMsg, sessionId, sessionType), 500);
        }
    }

    public void sendForwardMessages(String displayName, String inputMsg, List<String> sessionInfo, SessionTypeEnum sessionType, List<ChatMessageBean> messages) {
        ...
        for (int index = messages.size() - 1; index >= 0; index--) {
            ...
            for (String session : sessionInfo) {
                IMMessage forwardMessage = MessageBuilder.createForwardMessage(message.getMessageData().getMessage(), session, sessionType);
                MessageHelper.clearAitAndReplyInfo(forwardMessage);
    ...
    public void sendMultiForwardMessage(String displayName, String inputMsg, List<String> sessionInfo, SessionTypeEnum sessionType, List<ChatMessageBean> messages) {
        ...
        try {
            ...
            CommonRepo.writeLocalFileAndUploadNOS(localFile, msgInfo, "text", new FetchCallback<String>() {
                @Override
                public void onSuccess(@Nullable String param) {
                    if (param != null) {
                        String fileMD5 = EncryptUtils.md5(localFile);
                        MultiForwardAttachment attachment = MessageHelper.createMultiTransmitAttachment(displayName, mSessionId, param, iMessageList);
                        attachment.md5 = fileMD5;
                        for (String session : sessionInfo) {
                            IMMessage textMsg = MessageBuilder.createCustomMessage(session, sessionType, attachment);
                            sendMessage(textMsg, false, true);
    ...
    public void sendLocationMessage(ChatLocationBean locationBean) {
        ...
        IMMessage locationMessage = MessageBuilder.createLocationMessage(mSessionId, mSessionType, locationBean.getLat(), locationBean.getLng(), locationBean.getAddress());
    ...
    public void sendVideoMessage(File videoFile, long duration, int width, int height, String displayName) {
        if (videoFile != null) {
            ...
            IMMessage message = MessageBuilder.createVideoMessage(mSessionId, mSessionType, videoFile, duration, width, height, displayName);
    ...
    public void sendFileMessage(File docsFile, String displayName) {
        if (docsFile != null) {
            ...
            IMMessage message = MessageBuilder.createFileMessage(mSessionId, mSessionType, docsFile, displayName);
            sendMessage(message, false, true);
        }
    }
        ...
        if (ImageUtil.isValidPictureFile(mimeType)) {
            ...
            SendMediaHelper.handleVideo(uri, file -> {
                ...
                try {
                    ...
                    sendVideoMessage(file, Long.parseLong(duration), Integer.parseInt(width), Integer.parseInt(height), file.getName());
                } catch (Exception e) {
                    e.printStackTrace();
                } finally {
                    try {
    ...
    public void sendFile(Uri uri) {
        ...
        SendMediaHelper.handleFile(uri, file -> {
            try {
    ...
    public void sendMessage(IMMessage message, boolean resend, boolean needSendMessage) {
        if (message != null) {
            ...
            ChatRepo.sendMessage(message, resend, new FetchCallbackImpl<Void>() {
    ...
    public void initFetch(IMMessage anchor, boolean needToScrollEnd) {
        ...
        registerObservers();
        if (anchor == null) {
            GetMessagesDynamicallyParam dynamicallyParam = new GetMessagesDynamicallyParam(mSessionId, mSessionType);
            dynamicallyParam.setLimit(messagePageSize);
            dynamicallyParam.setDirection(GetMessageDirectionEnum.FORWARD);
            ChatRepo.getMessagesDynamically(dynamicallyParam, new FetchCallback<MessageDynamicallyResult>() {
                @Override
    ...
    public void fetchPinInfo() {
        ChatRepo.fetchPinInfo(mSessionId, mSessionType, 0L, new FetchCallback<MsgPinSyncResponseOptionWrapper>() {
            @Override
            public void onSuccess(@Nullable MsgPinSyncResponseOptionWrapper param) {
                ...
                if (param != null && param.isChanged()) {
                    ALog.d(LIB_TAG, TAG, "initFetch:fetchPinInfo:onSuccess:hasChange:" + param.isChanged() + ",size:" + param.getMsgPinInfoList().size());
    ...
    public void fetchMoreMessage(IMMessage anchor, GetMessageDirectionEnum direction, boolean needToScrollEnd) {
    ...
    public void fetchMessageListBothDirect(IMMessage anchor, boolean needToScrollEnd) {
        ...
        new Handler(Looper.getMainLooper()).post(() -> fetchMoreMessage(anchor, GetMessageDirectionEnum.FORWARD, needToScrollEnd));
    ...
    private void onListFetchSuccess(IMMessage anchorMsg, boolean needToScrollEnd, List<IMMessageInfo> param, GetMessageDirectionEnum direction) {
        ALog.d(LIB_TAG, TAG, "onListFetchSuccess -->> size:" + (param == null ? "null" : param.size()) + " direction:" + direction);

        LoadStatus loadStatus = (param == null || param.size() == 0) ? LoadStatus.Finish : LoadStatus.Success;
    ...
    private List<ChatMessageBean> convert(List<IMMessageInfo> messageList) {
        ...
        for (IMMessageInfo message : messageList) {
            if (mIsTeamGroup && message.getMessage().getAttachment() instanceof NotificationAttachment) {
                NotificationAttachment attachment = (NotificationAttachment) message.getMessage().getAttachment();
    ...
    public void replyMessage(IMMessage message, IMMessage replyMsg, boolean resend) {
        ...
        Map<String, Object> remote = MessageHelper.createReplyExtension(message.getRemoteExtension(), replyMsg);
        message.setRemoteExtension(remote);
        sendMessage(message, resend, true);
    }

    public void replyTextMessage(String content, IMMessage message, List<String> pushList, Map<String, Object> remoteExtension) {
        ALog.d(LIB_TAG, TAG, "replyTextMessage,message" + (message == null ? "null" : message.getUuid()));
        IMMessage textMsg = MessageBuilder.createTextMessage(mSessionId, mSessionType, content);
        MessageHelper.appendTeamMemberPush(textMsg, pushList);
        if (remoteExtension != null) {
            textMsg.setRemoteExtension(remoteExtension);
    ...
    private final Observer<MsgPinSyncResponseOption> msgPinAddObserver = msgPinSyncResponseOption -> {
        Pair<String, MsgPinOption> pinInfo = new Pair<>(msgPinSyncResponseOption.getKey().getUuid(), msgPinSyncResponseOption.getPinOption());
        addPinMessageLiveData.setValue(pinInfo);
    };

    private final Observer<MsgPinSyncResponseOption> msgPinRemoveObserver = responseOption -> removePinMessageLiveData.setValue(responseOption.getKey().getUuid());
    ...
    public void addMessagePin(IMMessageInfo messageInfo, String ext) {
        ...
        ChatRepo.addMessagePin(messageInfo.getMessage(), ext, new ChatCallback<Long>() {
            @Override
            public void onSuccess(@Nullable Long param) {
                ...
                Pair<String, MsgPinOption> pinInfo = new Pair<>(messageInfo.getMessage().getUuid(), pinOption);
                addPinMessageLiveData.setValue(pinInfo);
            }
        });
    }
    ...
    public void removeMsgPin(IMMessageInfo messageInfo) {
        ...
        ChatRepo.removeMessagePin(messageInfo.getMessage(), new ChatCallback<Long>() {
            @Override
            public void onSuccess(@Nullable Long param) {
                super.onSuccess(param);
