...
//      layoutParams.topMargin = BarUtils.getStatusBarHeight();
        binding.contactNewFragmentTopLl.setPadding(binding.contactNewFragmentTopLl.getPaddingLeft(), BarUtils.getStatusBarHeight(), binding.contactNewFragmentTopLl.getPaddingRight(), 0);
        ...
        binding.layoutNewFriend.setOnClickListener(view -> XKitRouter.withKey(RouterConstant.PATH_FUN_MY_NOTIFICATION_PAGE).withContext(requireContext()).navigate());
        binding.layoutMyGrop.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                MyGroupListActivity.start(MyGroupListActivity.class, getContext(), null);
            }
        });
        binding.layoutGropNotice.setOnClickListener(view -> XKitRouter.withKey(RouterConstant.PATH_FUN_MY_NOTIFICATION_PAGE).withParam("type", "1").withContext(requireContext()).navigate());

        binding.layoutKehu.setOnClickListener(view -> {
            XKitRouter.withKey(RouterConstant.PATH_FUN_CHAT_P2P_PAGE).withParam(RouterConstant.CHAT_ID_KRY, DataUtil.getKeFuId()).withContext(requireContext()).navigate();
        });
        _initViews();
        _requestData();
        ...
        HttpUtil.apiW().friends_applyListNum(new RegisterBean()).enqueue(new CommonCallback<NetData>() {
            ...
            public void Successful(Call<NetData> call, Response<NetData> response, NetData body) {
                ...
                if (applyNumBean.friendApplyNum > 0) {
                    binding.caontactListHeaderNewFriendNumTv.setVisibility(View.VISIBLE);
                    binding.caontactListHeaderNewFriendNumTv.setText(applyNumBean.friendApplyNum + "");
                } else {
                    binding.caontactListHeaderNewFriendNumTv.setVisibility(View.GONE);
                }
                if (applyNumBean.groupApplyNum > 0) {
                    binding.teamNonticeNum.setVisibility(View.VISIBLE);
                    binding.teamNonticeNum.setText(applyNumBean.groupApplyNum + "");
                } else {
                    binding.teamNonticeNum.setVisibility(View.GONE);
                }
                adapter.notifyDataSetChanged();
