...
class GrabEngine(
    ...
    fun shouldGrab(msg: RedInbound): DenyReason? {
        ...
        if (!config.masterSwitch || !config.isAuthorized()) return DenyReason.GLOBAL
    ...
    private fun performGrab(msg: RedInbound) {
        ...
        if (!config.masterSwitch || !config.isAuthorized() || !config.secondSwitch) {
            finishGrab(id)
            return
        }
