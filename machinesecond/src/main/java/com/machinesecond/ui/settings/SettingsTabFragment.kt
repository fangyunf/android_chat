...
class SettingsTabFragment : Fragment() {
    ...
    private fun rebuildViews() {
        ...
        for (row in rows) {
            if (row.key in SKIP_APPLY_KEYS && row.type != RowType.Description &&
                row.type != RowType.Action && row.type != RowType.Button
            ) {
                // still show
            }
            val view = SettingsRowInflater.inflate(
                ctx, row,
                onChanged = { changed ->
                    if (!MsSdk.getConfig().isAuthorized() &&
                        changed.key != "authCode" &&
                        changed.key !in UNGATED_KEYS
                    ) {
    ...
    companion object {
        ...
        private val NO_APPLY_KEYS = setOf(
            "authCode", "desc", "compensationMatrix", "discountMatrix",
            "normalFan", "preciseFan", "customFan", "grabLog", "diagLog", "ios14AuthTip"
        )
        private val SKIP_APPLY_KEYS = NO_APPLY_KEYS

        /** 九宫格/打折是纯本地矩阵，编辑不受授权拦截（对齐 iOS 只拦开关与密码） */
        private val UNGATED_KEYS = setOf("compensationMatrix", "discountMatrix")
